class MessageCopy < ActiveRecord::Base
  ...
  belongs_to :recipient, :class_name => MessageCopy::USER_MODEL, :foreign_key => :recipient_id
  belongs_to :sender, :class_name => MessageCopy::USER_MODEL, :foreign_key => :sender_id
  ...
  def all_recipients_without_sender
    MessageCopy::USER_MODEL.constantize.joins("INNER JOIN message_copies ON accounts.id = message_copies.recipient_id and message_copies.sender_id != message_copies.recipient_id and message_copies.message_id = #{self.message_id}")
  end
